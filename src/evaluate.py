"""
Evaluation Pipeline for Extracted Consent Fields

Checks coverage, emptiness, and basic consistency of the extracted
fields in data/processed/chunks/fields.
"""
...
def evaluate_fields(
    fields_dir: str = "data/processed/chunks/fields",
) -> Dict[str, Any]:
    fields_path = Path(fields_dir)
    files = list(fields_path.glob("*.json"))

    if not files:
        logger.info("No field files found in data/processed/chunks/fields")
        return {}
    ...
    logger.info("=" * 60)
    logger.info("Field Extraction Evaluation Results")
    logger.info("=" * 60)
